...
palette() {
    ...
    if [ $(cat $HOME/.config/.current-theme | tail -n1) = "dynamic" ]; then
        # Update fuzzel colors
        $HOME/.config/scripts/palette/update-fuzzel.sh "$(cat $HOME/.config/fuzzel/additional-options)"

        # Update mako colors
        $HOME/.config/scripts/palette/update-mako.sh "$(cat $HOME/.config/mako/additional-options)"
    fi
